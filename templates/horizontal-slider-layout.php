	...
	<ul id="slider-members-list" class="item-list item-list members-list slider-members-list" aria-live="polite" aria-relevant="all" aria-atomic="true">
		<li class="pane1">
			<div class="default demo-empty-slide">				
				<img src="<?php echo esc_url( BFFS_PLUGIN_URL . 'public/images/not-faund.png' ); ?>">
		...
		while ( bp_members() ) :
			...
			if ( bp_is_active( 'messages' ) ) {
				if ( ( function_exists( 'buddypress' ) && isset( buddypress()->buddyboss ) ) ) {
					$messgae_url = wp_nonce_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?r=' . bp_core_get_username( $member_id ) );
				} else {
					$messgae_url = wp_nonce_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?r=' . bp_members_get_user_slug( $member_id ) );
				}
			}
					...
					<div class="bffs_user_layout_one">
						...
						<div class="bffs_swipe_layout_bottom_meta">
							...
							<div class="bffs_item-title">
								<a href="<?php bp_member_permalink(); ?>"><?php echo esc_html( $member_name ); ?></a>
								<?php if ( bp_suggestions_is_user_online( $member_id ) ) : ?>
