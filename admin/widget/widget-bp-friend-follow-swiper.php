 ...
 * @since 1.0.0
 */
class BP_Friend_Follow_Swiper_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$name        = _x( 'BuddyPress Friend & Follow Swiper', 'widget name', 'buddypress-friend-follow-suggestion' );
		...
		parent::__construct(
			...
			array(
				...
				'classname'   => 'widget_bp_friend_follow_swiper_widget buddypress widget',
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$exclude_user = array();
		// exclude admin id from the swiper widget for the all users.
		$admins = get_users( array( 'role' => 'administrator' ) );
		if ( ! empty( $admins ) ) {
			$admin_id = $admins[0]->ID;
		}
		$exclude_user[] = $admin_id;
		// exclude remove user from the swiper widget.
		$bffs_remove_user = get_user_meta( bp_loggedin_user_id(), 'bffs_remove_user', true );
		if ( ! is_array( $bffs_remove_user ) ) {
			$bffs_remove_user = array();
		}

		$exclude_user = array_merge( $exclude_user, $bffs_remove_user );
			...
			<p>
				<label><?php esc_attr_e( 'Layout:', 'buddypress-friend-follow-suggestion' ); ?></label>
					<select name="<?php echo esc_attr( $this->get_field_name( 'layout' ) ); ?>">
						<option value="swiper_layout_one"<?php echo isset( $instance['layout'] ) ? selected( $instance['layout'], 'swiper_layout_one' ) : ''; ?>><?php esc_html_e( 'Swiper Layout One', 'buddypress-friend-follow-suggestion' ); ?></option>
						<option value="swiper_layout_two"<?php echo isset( $instance['layout'] ) ? selected( $instance['layout'], 'swiper_layout_two' ) : ''; ?>><?php esc_html_e( 'Swiper Layout Two', 'buddypress-friend-follow-suggestion' ); ?></option>
					</select>				
			</p>
			<p>
				<label>
...
function bp_swiper_register_widgets() {
	add_action(
		'widgets_init',
		function() {
			return register_widget( 'BP_Friend_Follow_Swiper_Widget' );
		}
	);
}
